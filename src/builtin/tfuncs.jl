...
TFUNCS["get_arr"] = function (argtypes::Vector{DataType})
    ...
    if argtypes[1] == MuTypes.AbstractArray
        return MuTypes.Any
    end

    return _get_array_eltype(argtypes[1])
end
...
TFUNCS["expanddims_arr"] = function (argtypes::AbstractArray)
    ...
    return MuTypes.Array{_get_array_eltype(argtypes[1]), _get_array_dim(argtypes[1]) + 1}
end
...
TFUNCS["sum_arr"] = function (argtypes::AbstractArray)
    ...
    eltype = _get_array_eltype(argtypes[1])
    ...
    return MuTypes.Array{eltype, dim}
end
