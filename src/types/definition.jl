...
# Nil is special type for `Union, Tuple` to represent end of type list.
# So Nil must not be appear in type of value, signature, etc.
struct Nil end
...
struct AbstractTuple <: MuType end
struct AbstractArray <: MuType end
struct Array{T <: MuType, N} <: MuType end
struct Union{S <: Base.Union{MuType, Nil}, T <: Base.Union{MuType, Nil}} <: MuType end
struct Tuple{S <: Base.Union{MuType, Nil}, T <: Base.Union{MuType, Nil}} <: MuType end
...
isconcrete(::Type{AbstractTuple}) = false
...
expand_types(::Type{Union{S, Nil}}) where {S} = [expand_types(S)]
expand_types(::Type{Union{Nil, Nil}}) = []
...
function _normalize(::Type{
    ...
    return Union{
        _normalize(Tuple{S1, T1}),
        _normalize(Tuple{S1, T2}), 
        _normalize(Tuple{S2, T1}), 
        _normalize(Tuple{S2, T2})
        }
end
